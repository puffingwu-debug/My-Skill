...
Python 运行环境：`~/.codex/skills/sanqu-roof-layout/.venv/bin/python`；依赖 `ezdxf`。SVG 预览由脚本直接生成，无需 Pillow、浏览器、matplotlib 或在线转换服务。

```sh
~/.codex/skills/sanqu-roof-layout/.venv/bin/python \
  ~/.codex/skills/sanqu-roof-layout/scripts/inspect_drawing.py \
...
`常州翡丽蓝湾8栋曲面瓦项目排布V6.dwg（历史案例，未随包发布）`

已完成 DWG 只读导出和 DXF 直接读取两条路径：导出 DXF 为 74,608,348 bytes，Core Console 返回 0，源 SHA-256 前后相同。模型空间展开统计为 176 图层、867 条文字、1,366 个闭合候选轮廓、172 条尺寸。`INSUNITS=0`。这些数量只用于识别回归，不是屋面数量或瓦片物料统计。

700 标注证据：源 INSERT handle `467C7`，嵌套块路径 `1- / 2`，显示原文 `700mm`，尺寸原存值 `700.0`，展开后几何测量 `14000.0`。原标注在放大块中，**不能把 14000 当作实际 14 m 尺寸，也不能只凭这一条详图标注断言屋面横向网格是700**。已目视核查局部SVG：该700标注属于含“汇流条1/2/3”和层压技术要求的电池构造详图，不是屋面瓦片重复间距尺寸链。用户已于2026-09-13确认现款发电瓦、主配瓦横向与正脊沿脊统一713 mm；本旧详图700不覆盖该确认。
